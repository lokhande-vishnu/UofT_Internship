...
predictors=train_data1(:,2:5601);
% indices = crossvalind('Kfold',size(response,1)/(6*8),5);
...
%Cross-Vlaidation has been explicitly performed
for i = 1:5
    i
%     test_set = repmat(test_set,(6*8),1);    train_set = repmat(train_set,(6*8),1);
    ...
    %Dimensionality Reduction Option A
%     options = [];
...
%     C = c*eigvector;
    ...
    %Dimensionality Reduction Option B
...
%     
%     
%     % Finding the most occuring class among groupable samples 
