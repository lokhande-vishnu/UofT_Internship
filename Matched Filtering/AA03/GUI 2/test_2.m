...
ind_G1G1_C1_first=zeros(30,28,size(EEG_C1_second_al,2));


for i=1:size(EEG_C1_second_al,2)
[C1_first,T] = SSN_(EEG_C1_second_al(1:4001,i));
for f=1:30
x(f,:)=C1_first(f,:);
y(f,:) = filter(b,1,x(f,:));
% Find matches
...
ind_G1G1_C2_first=zeros(30,28,size(EEG_C2_second_al,2));

[C2_first,T] = SSN_(EEG_C2_second_al(1:4001,i));
for f=1:30
x(f,:)=C2_first(f,:);
y(f,:) = filter(b,1,x(f,:));
% Find matches
