...
ind(f,handles.n(matches),1)=1;
end
for k=1:size(ind,2)
    
        G1G1_C1_G1G1(k,:)=abs(G1G1_C1_out_first(k,:,trial)-ind(k,:));  
 
        G1G1_C1_G1G2(k,:)=abs(G1G2_C1_second(k,:)-ind(k,:));

        G1G1_C1_G2G1(k,:)=abs(G2G1_C1_first(k,:)-ind(k,:)); 
        
        G1G1_C1_G2G2(k,:)=abs(G2G2_C1_second(k,:)-ind(k,:)); 
        
        G1G1_C1_G3G1(k,:)=abs(G3G1_C1_first(k,:)-ind(k,:)); 
           
        G1G1_C1_G3G2(k,:)=abs(G3G2_C1_second(k,:)-ind(k,:)); 
                    
end   
matrixG1G1(:,1)=sum(G1G1_C1_G1G1,2);
matrixG1G1(:,2)= sum(G1G1_C1_G1G2,2);
matrixG1G1(:,3)= sum(G1G1_C1_G2G1,2);
matrixG1G1(:,4)= sum(G1G1_C1_G2G2,2);
matrixG1G1(:,5)= sum(G1G1_C1_G3G1,2);
matrixG1G1(:,6)= sum(G1G1_C1_G3G2,2);
